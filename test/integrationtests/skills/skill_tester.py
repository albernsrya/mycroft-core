...
class SkillTest(object):
    ...
    def send_play_query(self, s, test_case):
        ...
        self.emitter.emit("play:query",
                          Message("play:query:", {"phrase": play_query}))
...
class EvaluationRule:
    ...
    def __init__(self, test_case, skill=None):
        ...
        if test_case.get("expected_dialog", None):
            if not skill:
                ...
                self.rule.append(["or"] +
                                 expected_dialog_check(expected_dialog, skill))
    ...
    def _partial_evaluate(self, rule, msg):
        ...
        if rule[0] == "match":
            if not (self._get_field_value(rule[1], msg) and re.match(
                    rule[2], self._get_field_value(rule[1], msg))):
                return False
