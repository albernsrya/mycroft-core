...
from mycroft.dialog import MustacheDialogRenderer, get, load_dialogs
    ...
    def test_general_dialog(self):
        ...
        for file in template_path.iterdir():
            if file.suffix == ".dialog":
                ...
                context = json.load(
                    file.with_suffix(".context.json").open("r",
                                                           encoding="utf-8"))
                self.assertEqual(
    ...
    def test_multiple_dialog(self):
        ...
        for file in template_path.iterdir():
            if file.suffix == ".dialog":
                ...
                self.assertIn(self.stache.render(file.name, context=context),
                              results)
    ...
    def test_comment_dialog(self):
        ...
        for f in template_path.iterdir():
            if f.suffix == ".dialog":
                ...
                results = [
                    line.strip() for line in f.with_suffix(".result").open("r")
                ]
                # Try all lines
                for index, line in enumerate(results):
