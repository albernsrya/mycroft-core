...
from mycroft.skills.skill_data import (load_regex, load_regex_from_file,
...
class TestMycroftSkill(unittest.TestCase):
    ...
    def test_load_regex_from_file_multiple(self):
        self.check_regex_from_file(
            "valid/multiple.rx",
            ["(?P<AMultipleTest1>.*)", "(?P<AMultipleTest2>.*)"])
    ...
    def test_load_vocab_from_file_single_alias(self):
        self.check_read_vocab_file(
            join(vocab_base_path(), "valid/singlealias.voc"),
            [["water", "watering"]])
    ...
    def _test_intent_file(self, s):
        ...
        expected_results = [
            {
                "file_name":
                join(dirname(__file__), "intent_file", "vocab", "en-us",
                     "test.intent"),
                "name":
                str(s.skill_id) + ":test.intent",
            },
            {
                "file_name":
                join(
    ...
    def test_voc_match(self):
        ...
        self.assertTrue(
            s.voc_match("would you please turn off the lights",
                        "turn_off_test"))
        ...
        self.assertFalse(
            s.voc_match("My hovercraft is full of eels", "turn_off_test"))
    ...
    def test_translate_locations(self):
        ...
        s = SimpleSkill1()
        s.root_dir = abspath(join(dirname(__file__), "translate",
                                  "in-dialog/"))
