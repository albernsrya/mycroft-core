...
def setup_service(service_module, config, bus):
    ...
    """
    if hasattr(service_module, "autodetect") and callable(
...
def load_services(config, bus, path=None):
    ...
    Returns:
        List of started services.
    """
    return load_internal_services(config, bus, path) + load_plugins(
        config, bus)
...
class AudioService:
    ...
    def track_start(self, track):
        ...
        if track:
            ...
            self.bus.emit(
                Message("mycroft.audio.playing_track", data={"track": track}))
        else:
    ...
    def _perform_stop(self):
        ...
        if self.current:
            ...
            if self.current.stop():
                self.bus.emit(
                    Message("mycroft.stop.handled", {"by": "audio:" + name}))
    ...
    def _play(self, message):
        ...
        with self.service_lock:
            ...
            for s in self.service:
                if "utterance" in message.data and s.name in message.data[
                        "utterance"]:
                    prefered_service = s
