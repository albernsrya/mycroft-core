...
from mycroft.util import (check_for_signal, get_ipc_directory, play_wav,
...
class MutableMicrophone(Microphone):
    ...
    def _start(self):
        ...
        assert (self.stream is
                None), "This audio source is already inside a context manager"
    ...
    def recording_complete(self):
        ...
        if too_much_silence:
            LOG.debug("Too much silence recorded without start of sentence "
                      "detected")
        return (self._quiet_enough()
                and self.silence_duration > self.silence_after_loud) and (
...
class ResponsiveRecognizer(speech_recognition.Recognizer):
    ...
    def _wait_until_wake_word(self, source, sec_per_buffer):
        ...
        self._listen_triggered = False
        return WakeWordData(audio_data, said_wake_word, self._stop_signaled,
                            ww_frames)
