...
class EnclosureAPI:
    ...
    def eyes_fill(self, percentage):
        ...
        if percentage < 0 or percentage > 100:
            raise ValueError("percentage ({}) must be between 0-100".format(
                str(percentage)))
        self.bus.emit(
            Message(
                "enclosure.eyes.fill",
    ...
    def eyes_volume(self, volume):
        ...
        if volume < 0 or volume > 11:
            raise ValueError("volume ({}) must be between 0-11".format(
                str(volume)))
        self.bus.emit(
            Message(
