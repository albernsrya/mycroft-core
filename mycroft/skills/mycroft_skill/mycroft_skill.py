...
from mycroft.util import (camel_case_split, play_audio_file,
...
class MycroftSkill:
    ...
    def _init_settings(self):
        ...
        if not self.settings_write_path.joinpath("settings.json").exists():
            self.settings_write_path = Path(
                BaseDirectory.save_config_path("mycroft", "skills",
        ...
        if not settings_read_path.joinpath("settings.json").exists():
            for dir in BaseDirectory.load_config_paths("mycroft", "skills",
                                                       basename(
                                                           self.root_dir)):
                path = Path(dir)
    ...
    def _register_public_api(self):
        ...
        for key in self.public_api:
            if "type" in self.public_api[key] and "func" in self.public_api[
                    key]:
    ...
    def _register_system_event_handlers(self):
        ...
        self.events.add("mycroft.skills.settings.changed",
    ...
    def register_intent(self, intent_parser, handler):
        ...
        if isinstance(intent_parser,
                      str) and intent_parser.endswith(".intent"):
    ...
    def remove_cross_skill_context(self, context):
        ...
        if not isinstance(context, str):
            raise ValueError("context should be a string")
        self.bus.emit(
            Message("mycroft.skill.remove_cross_context",
                    {"context": context}))
    ...
    def speak(self, utterance, expect_response=False, wait=False, meta=None):
        ...
        m = message.forward("speak", data) if message else Message(
            "speak", data)
        self.bus.emit(m)
    ...
    def schedule_event(self,
        ...
        return self.event_scheduler.schedule_event(handler,
                                                   when,
                                                   data,
                                                   name,
                                                   context=context)
    ...
    def schedule_repeating_event(self,
        ...
        message = dig_for_message()
        context = context or message.context if message else {}
        return self.event_scheduler.schedule_repeating_event(handler,
                                                             when,
                                                             frequency,
                                                             data,
                                                             name,
