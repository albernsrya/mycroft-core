...
def _shutdown_skill(instance):
    ...
    try:
        ...
        if shutdown_time > 1:
            LOG.warning("{} shutdown took {} seconds".format(
                instance.skill_id, shutdown_time))
    except Exception:
...
class SkillManager(Thread):
    ...
    def _update_skills(self):
        ...
        do_skill_update = (time() >= self.skill_updater.next_download
                           and self.skills_config["auto_update"])
        if do_skill_update:
            self.skill_updater.update_skills()
        ...
        for skill_loader in self.skill_loaders.values():
            if skill_loader.skill_id == skill_id:
                ...
                try:
                    ...
                    if len(
                            signature(skill_loader.instance.converse).
                            parameters) == 1:
