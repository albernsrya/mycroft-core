...
class EventScheduler(Thread):
    ...
    def check_state(self):
        ...
        with self.event_lock:
            ...
            for event in self.events:
                ...
                passed = [(t, r, d, c) for (t, r, d, c) in e
                          if t <= current_time]
                # and remaining times that we're still waiting for
                remaining = [(t, r, d, c) for t, r, d, c in e
                             if t > current_time]
    ...
    def schedule_event(self,
        ...
        with self.event_lock:
            ...
            if repeat and event in self.events:
                LOG.debug(
                    "Repeating event {} is already scheduled, discarding".
                    format(event))
            else:
    ...
    def clear_empty(self):
        ...
        with self.event_lock:
            self.events = {
                k: self.events[k]
                for k in self.events if self.events[k] != []
            }
